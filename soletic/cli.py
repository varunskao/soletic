...
def _is_api_key_defined():
    if not os.getenv("HELIUS_API_KEY"):
        click.echo(
            "HELIUS_API_KEY not found. Please define HELIUS_API_KEY in your .env file.",
            err=True,
        )
        return False
    return True
...
@click.option(
    ...
    "--network",
    default=None,
    help='The choice of network: "mainnet" or "devnet".',
)
...
def setup(network, use_cache, log_file):
    ...
    if config and valid_config:
        ...
        if not click.confirm(
            "Would you like to update your configuration?", default=False
        ):
    ...
    config.update({"network": network, "cache": use_cache, "log_file": log_file})
...
@click.option(
    "--log-file", default=None, help="Update the file to which logs are written."
)
@click.pass_context
def update(ctx, network, use_cache, log_file):
    ...
    if not ctx.obj:
        click.echo(
            "Must run `soletic setup` before `soletic update` to configure the tool."
        )
        return
...
@click.option(
    ...
    "--network",
    default=None,
    help='Override the configuration setting, if required: "mainnet" or "devnet".',
)
@click.option(
...
@click.option(
    "--ignore-cache", is_flag=True, default=False, help="Override cache setting."
)
@click.option(
    "-f",
    "--format",
    default="unix",
    help='Default format is unix time. Options are: "unix" or "datetime".',
)
@click.argument("program_address", type=str)
@click.pass_context
def getProgramDeploymentTime(
    ctx, program_address, network, verbose, debug, ignore_cache, format
):
    """
    ...
    if not ctx.obj:
        click.echo(
            "Must run `soletic setup` before `soletic commands` to configure the tool."
        )
        return
    ...
    spa = SolanaProgramAnalyzer(
        log_file=ctx.obj["log_file"], verbose=verbose, debug=debug
    )
    response_or_err = spa.get_deployment_timestamp(
        program_address=program_address, network=network, use_cache=cache
    )
    if isinstance(response_or_err, int):
        ...
        if format == "datetime":
            click.echo(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(response_or_err))
            )
    else:
        click.echo(response_or_err)
